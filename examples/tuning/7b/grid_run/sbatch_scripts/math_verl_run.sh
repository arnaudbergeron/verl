...
# Run Logging Config
NOW=$(date +%Y%m%d)
export WANDB_DIR=math-4shot-lora-qwen2.5-7b
export WANDB_PROJECT=${WANDB_DIR}
export WANDB_EXP=adv-${adv_estimation}-${loss_name}-o${outer_loop_size}-lr${learning_rate}-bsz${bsz}-prob-${prob_granularity}-${loss_agg}-scale${sratio}-IS-correction----0.5b-SGD
...
python3 -m verl.trainer.main_ppo \
        ...
        data.train_files=${SCRATCH}/verl/data/math_4shot/train.parquet \
        data.val_files=${SCRATCH}/verl/data/math_4shot/test.parquet \
